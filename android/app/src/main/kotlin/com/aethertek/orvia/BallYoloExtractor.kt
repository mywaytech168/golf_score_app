...
class BallYoloExtractor(assetManager: AssetManager) {
    ...
    // ════════════════════════════════════════════════════════════════
    // p0-SAHI：擊球前數幀對整幀做網格 SAHI 掃描，跨幀群聚出最穩定的靜止球 = P0。
    //
    // 與 extract()（單 tile 跟 ROI、逐幀）不同：此處在擊球前的少數幀上，把整幀切成
    // 重疊的 640 tile 全覆蓋掃描（對齊 Python SAHI），專門解決「不知道球在哪」的 p0
    // 偵測。回傳的 P0 由 Dart 端餵給 BallTracker 當種子，飛行段仍走幀差+Kalman。
    //
    // 回傳 { "cx":Int, "cy":Int, "frame":Int, "conf":Double, "count":Int }（coded 空間）
    // 或 null。⚠️ 此方法未經裝置編譯/實跑驗證，需上機調 PRE_WINDOW / 網格步長 / conf。
    // ════════════════════════════════════════════════════════════════
    ...
    fun findP0Sahi(
        ...
        if (!java.io.File(inputPath).exists()) { Log.w(TAG, "[p0Sahi] file not found"); return null }

        val extractor = MediaExtractor()
        try { extractor.setDataSource(inputPath) }
        catch (e: Exception) { Log.e(TAG, "[p0Sahi] open fail: $e"); return null }
        ...
        if (videoTrack < 0 || inputFormat == null) { extractor.release(); return null }
        extractor.selectTrack(videoTrack)

        val videoW = inputFormat.getInteger(MediaFormat.KEY_WIDTH)
        val videoH = inputFormat.getInteger(MediaFormat.KEY_HEIGHT)
        val videoMime = inputFormat.getString(MediaFormat.KEY_MIME) ?: "video/avc"
        val fps = runCatching { inputFormat.getInteger(MediaFormat.KEY_FRAME_RATE).toDouble() }.getOrElse { 30.0 }
        val hitFrame = if (hitSec != null && fps > 0) (hitSec * fps).toInt() else -1

        // 搜尋窗：擊球前 PRE_WINDOW 幀 ~ 擊球幀（靜止球此時在 tee 上）。無 hitSec 時取前段。
        val preWindow = 8
        ...
        val step = BallYoloDetector.INPUT_SIZE / 2          // 320：50% 重疊
        val half = BallYoloDetector.INPUT_SIZE / 2
        // 群聚累積：每個 cluster = [sumX, sumY, sumConf, count, bestFrame, bestConf]
        ...
        fun addDet(cx: Float, cy: Float, conf: Float, frame: Int) {
            for (c in clusters) {
                val dx = cx - (c[0] / c[3]); val dy = cy - (c[1] / c[3])
                if (dx * dx + dy * dy <= 40.0 * 40.0) {  // R=40px
                    c[0] += cx; c[1] += cy; c[2] += conf; c[3] += 1.0
                    if (conf > c[5]) { c[5] = conf.toDouble(); c[4] = frame.toDouble() }
                    return
                }
            }
            clusters.add(doubleArrayOf(cx.toDouble(), cy.toDouble(), conf.toDouble(), 1.0, frame.toDouble(), conf.toDouble()))
        }
            ...
            while (true) {
                if (!inputEos) {
                    ...
                    if (inIdx >= 0) {
                        ...
                        val size = extractor.readSampleData(buf, 0)
                        if (size < 0) {
                            decoder.queueInputBuffer(inIdx, 0, 0, 0, MediaCodec.BUFFER_FLAG_END_OF_STREAM); inputEos = true
                        } else {
                            decoder.queueInputBuffer(inIdx, 0, size, extractor.sampleTime, 0); extractor.advance()
                        }
                    }
                }
                val outIdx = decoder.dequeueOutputBuffer(bufInfo, 10_000L)
                if (outIdx == MediaCodec.INFO_TRY_AGAIN_LATER || outIdx == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) continue
                if (outIdx < 0) continue
                        ...
                        while (ty <= videoH - half) {
                            ...
                            while (tx <= videoW - half) {
                                ...
                                val dets = detector.detect(
                                    ...
                                    vRawBuf, vP.rowStride, vP.pixelStride,
                                    videoW, videoH, tx, ty,
                                    confThreshold = 0.20f,
                                    tileEdgeMargin = 20f,
                                )
        ...
        if (clusters.isEmpty()) { Log.w(TAG, "[p0Sahi] no detections in window [$winStart,$winEnd]"); return null }
        // 最佳 cluster：總信心最高（出現越多次、越有信心 = 越像靜止球）
        val best = clusters.maxByOrNull { it[2] }!!
        val cx = (best[0] / best[3]).toInt()
        val cy = (best[1] / best[3]).toInt()
        val count = best[3].toInt()
        val conf = best[2] / best[3]
        val frame = best[4].toInt()
        Log.d(TAG, "[p0Sahi] P0=($cx,$cy) frame=$frame count=$count avgConf=${"%.2f".format(conf)} clusters=${clusters.size}")
        onProgress?.invoke("findP0Sahi", 1.0, "P0 偵測完成", 1, 1)
        return mapOf("cx" to cx, "cy" to cy, "frame" to frame, "conf" to conf, "count" to count)
    }
