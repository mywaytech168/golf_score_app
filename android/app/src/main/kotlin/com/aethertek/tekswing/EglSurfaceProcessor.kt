...
class EglSurfaceProcessor(private val encoderInputSurface: Surface) {
    ...
    fun awaitAndRender(rotationDeg: Int, dstWidth: Int, dstHeight: Int, ptsUs: Long,
        ...
        // MVP：旋轉 + 水平翻轉（前鏡頭）
        // ★ 正號（+rotationDeg）：
        //   OpenGL rotateM 正方向 = CCW（逆時針）
        //   MVP 旋轉的是 quad，content 轉向與 quad 相反：
        //   rotateM(+90°) → quad CCW → content CW 90° → 符合 rotation=90 metadata
        //   rotateM(-90°) → quad CW  → content CCW 90° → 輸出 -90°（舊 bug）
        val mvp = FloatArray(16)
        Matrix.setIdentityM(mvp, 0)
        Matrix.rotateM(mvp, 0, rotationDeg.toFloat(), 0f, 0f, 1f)   // ← +號，不是 -號
        if (flipHorizontal) {
            // X 軸 scale = -1 → 水平翻轉
