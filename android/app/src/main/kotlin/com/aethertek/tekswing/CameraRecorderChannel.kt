...
class CameraRecorderChannel(
    ...
    companion object {
        ...
        const val METHOD_CHANNEL = "com.aethertek.tekswing/camera_recorder"
        const val POSE_CHANNEL   = "com.aethertek.tekswing/pose_landmarks"
        ...
        // analysisReader 超過此毫秒數沒輸出 → 判定 stale，改由 preview 降頻補 MediaPipe。
        // analysis 約 5fps（每 200ms 一幀），1000ms 無輸出才視為停擺。
    ...
    // ── MediaPipe ─────────────────────────────────────────────────────────────
    private val poseHelper = MediaPipePoseHelper(context) { lms, ts ->
        lastLandmarks = lms
    ...
    @Volatile private var analysisFrameSeq = 0
    ...
    private fun openCamera(facing: Int, quality: String, fps: Int, result: MethodChannel.Result) {
        ...
        cameraHandler.post {
            ...
            try {
                ...
                releaseOp("openCamera")
                replyOpenError(result, e.message ?: "CameraAccessException")
            }
        }
        } // mainHandler.post
    }
    ...
    private fun setupImageReaders(producer: TextureRegistry.SurfaceProducer) {
        ...
        // ── previewReader：顯示管線 ──────────────────────────────────────────
        previewReader = ImageReader.newInstance(PREVIEW_W, PREVIEW_H, ImageFormat.YUV_420_888, 3)
            .also { ir ->
                ir.setOnImageAvailableListener({ reader ->
                    ...
                    if (!isRendering) {
                        isRendering = true
                        renderHandler.post {
                            onDisplayNv21(data, PREVIEW_W, PREVIEW_H, sensorOrientation, isFront, producer)
                        }
                    }
                    ...
                    if (analysisFallbackFromPreview || analysisStale) {
                        ...
                        if (seq % 6 == 0 && !isAnalyzing) {
                            isAnalyzing = true
                            renderHandler.post {
                                onAnalysisNv21(data, PREVIEW_W, PREVIEW_H, sensorOrientation, isFront, tsNs)
                            }
                        }
                    }
        ...
        analysisReader = ImageReader.newInstance(ANALYSIS_W, ANALYSIS_H, ImageFormat.YUV_420_888, 4)
            .also { ir ->
                ir.setOnImageAvailableListener({ reader ->
                    var img: Image? = null
                    var nv21: ByteArray? = null
                    ...
                    val analysisSeq = (analysisFrameSeq++) % 6
                    if (analysisSeq == 0 && !isAnalyzing) {
                        isAnalyzing = true
                        val tsNs = imgTimestampNs
                        renderHandler.post {
                            onAnalysisNv21(data, ANALYSIS_W, ANALYSIS_H, sensorOrientation, isFront, tsNs)
                        }
                    }
                }, imageHandler)
            }
    ...
    private fun onDisplayNv21(
        ...
        try {
            ...
            if (lms.isNotEmpty()) {
                SkeletonRenderer.draw(android.graphics.Canvas(fullBmp), lms, fullBmp.width, fullBmp.height)
            }
    ...
    private fun onAnalysisNv21(
        ...
        try {
            ...
            // 旋轉至直式（640×360 → 360×640）
            val portraitBmp = if (rotation == 0 && !isFront) raw else {
                val m = Matrix().apply {
                    postRotate(rotation.toFloat())
                    if (isFront) postScale(-1f, 1f, raw.width / 2f, raw.height / 2f)
                }
