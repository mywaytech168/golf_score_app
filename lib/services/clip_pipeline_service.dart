...
class ClipPipelineService {
  ...
  static Future<ClipResult?> _processHit({
    ...
    if (!await File(csvPath).exists()) {
      debugPrint('[Pipeline] hit ${hit.hitIndex} → ❌ CSV 不存在：$csvPath');
    } else {
      ...
      // 確認輸出檔確實存在（Kotlin 端有時回傳 true 但檔案未正確寫入）
      if (overlaid != null && !await File(overlaid).exists()) {
        debugPrint('[Pipeline] hit ${hit.hitIndex} → ❌ 骨架輸出檔不存在，視為失敗');
        overlaid = null;
      }
      if (overlaid != null) {
        hasSkeleton = true;
    ...
    if (skeletonPath != null) {
      ...
      if (extraction == null) {
        ...
        debugPrint('[Pipeline] hit ${hit.hitIndex} → ✅ blob 提取完成：'
        ...
        if (trackPts.length >= 2) {
          ...
          debugPrint('[Pipeline] hit ${hit.hitIndex} → ⚠️ 軌跡點不足（${trackPts.length}），略過疊加');
        }
      }
    } else {
      debugPrint('[Pipeline] hit ${hit.hitIndex} → ⚠️ 骨架失敗，略過球軌跡');
    }
